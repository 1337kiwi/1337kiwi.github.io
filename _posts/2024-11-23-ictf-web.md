...
Tracing the pathway that CTF_FLAG is used, I see an admin() function that renders a template with the flag: `return render_template_string(admin_page_template, FLAG=CTF_FLAG, username="Admin", chat_history=chat_history)`
...
After trying some SSTI payloads with no luck, I decided to look deeper at the rest of the code. Specifically, how to trigger an admin page. While looking, I read over the `send_message()` function. 
...
Huh. chat[0] is marked as "safe", which means it won't be automatically escaped by Jinja. This means we can inject scripts in chat[0] and they will execute. I set up a local netcat listener, and sent a payload which should exfiltrate the admin cookie.
...
Now that I was able to obtain the admin cookie locally, I needed to exfiltrate it remotely. I set up a simple listener using https://webhook.site, and sent the same payload.
